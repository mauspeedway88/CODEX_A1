...
# === CONTADOR EN MEMORIA (ULTRA-RÁPIDO, CERO DISCO) ===
# Solo sube cuando llega una visita real. Cloudflare cachea la respuesta.
_visitas_counter = 0

# --- SISTEMA DE AUTENTICACIÓN (EL GUARDIÁN) ---
JWT_SECRET = "M4r1yn_Z3r0_TrusT_K3y_2026!"

def base64url_encode(data):
    return base64.urlsafe_b64encode(data).rstrip(b'=').decode('utf-8')

def generar_jwt(payload, expiration_minutes=1440): # 24 horas
...
class NetmarlynTrackerHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        # Silenciar los logs repetitivos de telemetría
        msg = args[0] if args else ""
        if "/api/live_stats" in msg:
            return
        super().log_message(format, *args)

    def do_GET(self):
        # === ENDPOINT TELEMETRÍA (IDÉNTICO AL ORIGINAL) ===
        if self.path == '/api/live_stats':
            try:
                ...
                self.end_headers()
                return
        # Para todo lo demás, servir archivos estáticos normalmente
        super().do_GET()
    ...
    def do_POST(self):
        ...
        if self.path == '/api/visitas':
            ...
            try:
                ...
                # Capturar IP real ANTES de guardar (Cloudflare inyecta CF-Connecting-IP)
                ip_real = (
                    self.headers.get('CF-Connecting-IP') or
                    self.headers.get('X-Forwarded-For', '').split(',')[0].strip() or
                    self.client_address[0]
                )
                country = self.headers.get('CF-IPCountry', 'N/A')
                telemetry_data['ip_real'] = ip_real
                telemetry_data['country'] = country
                ...
                # === INCREMENTAR CONTADOR EN MEMORIA (instantáneo) ===
                global _visitas_counter
                _visitas_counter += 1
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚡ VISITA #{_visitas_counter}: {is_new} | IP: {ip_real} | País: {country} | OS: {os_type}")
            ...
            try:
                ...
                token_payload = verificar_jwt(datos.get('token'))
                if not token_payload or token_payload.get('colegio') != codigo:
                    raise ValueError("Acceso Denegado: Token JWT inválido o expirado para este Silo.")
            ...
            try:
                ...
                token_payload = verificar_jwt(datos.get('token'))
                if not token_payload or token_payload.get('colegio') != codigo_colegio:
                    raise ValueError("Acceso Denegado: Token JWT inválido o expirado para este Silo.")
            ...
            # ===================================================
            # ENDPOINT: Validar credenciales de un colegio y emitir JWT
            # ===================================================
            try:
                content_length = int(self.headers.get('Content-Length', 0))
                if content_length > 0:
                            ...
                            if os.path.exists(config_path):
                                with open(config_path, "r", encoding="utf-8") as f:
                                    ...
                                    if config.get('id') == codigo and config.get('password_portal') == password:
                                        token = generar_jwt({"colegio": codigo, "nombre": config.get('nombre')})
                                        resultado = {
                                            "status": "ok",
    ...
    def do_OPTIONS(self):
        # [V43.5 FIX CRÍTICO] CORS Preflight — SIN este header el browser bloquea el POST a /api/chat
        # El browser envía OPTIONS antes de cualquier POST cross-origin. Sin Allow-Origin: * la petición
        # muere silenciosamente ANTES de llegar al endpoint, causando "Error de conexión".
        self.send_response(200)
        self.send_header('Access-Control-Allow-Origin', '*')
    ...
    def do_GET(self):
        ...
        # --- ENDPOINT TTS SERVER-SIDE (EDGE-TTS) ---
        if clean_path == '/api/tts':
            ...
            # Comando edge-tts. Devuelve MP3 por stdout.
            # --rate=+18%: velocidad natural de un maestro hablando (default es muy lento/pausado)
            cmd = [
                "/Users/mauricio/Library/Python/3.9/bin/edge-tts",
                "--text", text,
            ...
            try:
                ...
                stdout, stderr = process.communicate(timeout=30)  # 30s para textos largos
                ...
                print("[EDGE-TTS ERROR] Timeout")
                process.kill()
                self.send_response(504)
                self.end_headers()
            except Exception as e:
                print(f"[EDGE-TTS ERROR CRÍTICO] {e}")
                self.send_response(500)
                self.end_headers()
            return
