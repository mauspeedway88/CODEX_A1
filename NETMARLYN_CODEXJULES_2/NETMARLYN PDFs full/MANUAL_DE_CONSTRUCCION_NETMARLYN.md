...
* **Escala Masiva (El Secreto):** Con un `Edge TTL = 1 segundo` en la caché de Cloudflare, la Mac Mini está protegida. Aunque 50 mil usuarios hagan polling a 500ms, el servidor físico solo recibe **1 petición por segundo**.
...
## ✅ MÓDULO 10.1: Mandato de Escalabilidad (Miles de Archivos)
- **Desafío:** 50,000 maestros generando 1 infografía cada 50 minutos = **Miles de archivos**.
...
- **Razón Técnica:** Repartir la carga entre miles de carpetas evita que el sistema de archivos de macOS se vuelva lento. 5 miles de archivos en una carpeta rompen el disco; 5 miles repartidos en 10,000 silos es pan comido para el Mac Mini.
...
## ✅ SECCIÓN 14: ARQUITECTURA HÍBRIDA MASIVA: MAUNET PARA 50 mil alumnos

> [!IMPORTANT]
> **REGLA ESTRUCTURAL (Hive Mind & Edge Sync — Motor de Conceptos Educativos):** Esta arquitectura 98/2 aplica EXCLUSIVAMENTE al **motor de conceptos educativos** (`concepts.json` offline en los celulares). La infraestructura central de la Mac Mini M1 NO procesa los 50 mil de chats LLM concurrentemente. **Maunet Original con Ollama local** opera a escala de colegios piloto, no a escala nacional masiva. La carga de conceptos se transfiere a los celulares mediante la arquitectura "98/2" descrita abajo.
...
4. **Sincronización Desfasada:** Durante las siguientes horas, los 50 mil de celulares se conectan a NetMarlyn (cada uno en minutos y segundos diferentes, distribuyendo la carga de manera orgánica). Al detectar que el archivo Maestro creció, descargan **solo el incremento** (delta).
...
1. **Identificación Previa del Maestro (Llave de Acceso):** Para evitar descargar la base de datos de 50 mil alumnos, el maestro debe escanear **su propio carnet** al entrar a "Asistencia". El sistema lee su NM-ID y determina a qué colegio pertenece.
...
5. **Resiliencia Total:** Nunca se elimina un archivo de la cola sin confirmación de inserción en DB. Sistema particionado por colegio para evitar cuellos de botella globales. Capaz de escalar a **miles de registros concurrentes**.
...
### 🧠 MEMORIA PERSISTENTE DE 4 DÍAS (Maunet Original) — ARQUITECTURA APROBADA
**Aplica SOLO a Maunet Original (`MAUNET_SITE_SS_2`).** Maunet Público NO tiene esta funcionalidad.

> ⚠️ CORRECCIÓN DE ESCALA: El sistema de memoria conversacional de Maunet Original NO está dimensionado para 50 mil usuarios simultáneos. La arquitectura 98/2 (Hive Mind) aplica al motor de conceptos educativos (offline). Maunet Original con Ollama local opera a escala de colegios piloto activos.

**Arquitectura de 2 niveles:**
- **Corto plazo (RAM):** Últimos 13 turnos (exclusivamente de respuestas de Maunet) de la sesión activa en la terminal TTS. Volátil, se pierde al cerrar pestaña.
- **Largo plazo (JSON en servidor):** Resúmenes semánticos de sesiones cerradas, últimos 4 días. Se elimina automáticamente al superar los 4 días.

**El resumen semántico:**
- Cada sesión cerrada → UN resumen de 18-22 palabras, coherente y contextual (NO lista suelta)
- Ejemplo: `"explicación de fotosíntesis en plantas, ciclo del agua y mariposas monarca"`
- Generado por Ollama al cerrar sesión. JS valida el rango de palabras.
- Tamaño estimado por alumno: ~8KB máximo (4 días × 5 sesiones × ~40 bytes)

**Flujo:**
1. Alumno abre Maunet → JS descarga JSON del servidor (`GET /api/memory/{carnet}`) → guarda en RAM.
2. Alumno platica normalmente. Si pregunta del pasado reciente (misma sesión) → responde desde RAM limitando el contexto a los últimos 13 turnos (de respuestas de Maunet). Si pregunta de días anteriores → responde desde JSON. Si no pregunta → no lo toca.
3. Alumno cierra/sale → Ollama genera resumen semántico de 20 palabras → JS agrega entrada al JSON → limpia entradas >4 días → envía con `sendBeacon`.
4. **Seguro extra:** Auto-sync cada 5 minutos por si el navegador muere sin disparar el cierre.
...
**Estado:** ✅ DISEÑO APROBADO — Implementación pendiente. Ver Sesión-046 de la BITÁCORA.
